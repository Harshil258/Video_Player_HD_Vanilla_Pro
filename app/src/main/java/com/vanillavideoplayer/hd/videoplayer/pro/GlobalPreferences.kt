...
//        return false
    ...
    fun setNewVideosReminderList(context: Context, videoList: ArrayList<Long>) {
        val gson = Gson()
        val jsonString = gson.toJson(videoList)
        getSP(context).edit().putString("reminded_new_videos", jsonString).apply()
    }

    fun getNewVideosReminderList(context: Context): ArrayList<Long> {
        val gson = Gson()
        val jsonString = getSP(context).getString("reminded_new_videos", null)
        return if (jsonString != null) {
